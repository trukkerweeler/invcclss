...
# Configuration
PRIMARY_SCAN_DIRECTORY = r"C:\Users\tim\OneDrive\Documents\Work\CI\APScans"
# Use SCAN_PATH from .env if primary directory doesn't exist
SCAN_DIRECTORY = (
    SCAN_PATH
    if SCAN_PATH
    and os.path.exists(SCAN_PATH)
    and not os.path.exists(PRIMARY_SCAN_DIRECTORY)
    else PRIMARY_SCAN_DIRECTORY
)
...
def extract_po_number(text: str) -> List[str]:
    ...
    for pattern in PO_PATTERNS:
        ...
        matches = re.findall(pattern, collapsed_upper, re.IGNORECASE | re.MULTILINE)
        if not matches:
...
def extract_amount_from_text(
    text: str, supplier: Optional[str] = None
) -> Optional[str]:
    ...
    if supplier and supplier in po_profiles:
        ...
        for pattern in amount_patterns:
            ...
            try:
                ...
                match = re.search(
                    rf"{re.escape(pattern)}\s*[:\$\-]*\s*([0-9,]+\.?[0-9]*)",
                    text,
                    re.IGNORECASE,
                )
                if match:
...
def extract_po_with_supplier_profile(
    text: str, supplier: Optional[str] = None
) -> List[str]:
    ...
    if supplier == "BLAN1":
        ...
        if not match:
            ...
            print(f"[DEBUG BLAN1] Original text contains '5590': {'5590' in text}")
            # Show lines that contain digits and "0040702"
            if "0040702" in text:
                print(f"[DEBUG BLAN1] Found '0040702' in text")
                # Find context around it
                idx = text.find("0040702")
                print(f"[DEBUG BLAN1] Context: ...{text[max(0, idx-50):idx+50]}...")
...
def open_supplier_profile_manager():
    ...
    ttk.Label(
        root,
        text="Supplier Training Profiles & PO Detection",
        font=("Arial", 14, "bold"),
    ).pack(pady=10)
    ...
    ttk.Label(
        class_frame,
        text="Training samples for supplier classification (for gui.py)",
        font=("Arial", 10),
        foreground="gray",
    ).pack(pady=5)
    ...
    scrollbar1 = ttk.Scrollbar(class_frame, orient="vertical", command=canvas1.yview)
    ...
    if profiles:
        for supplier, samples in profiles.items():
            ttk.Label(
                scroll_frame1,
                text=f"{supplier}: {len(samples)} samples",
                font=("Arial", 10),
            ).pack(anchor=tk.W, pady=5)
    else:
        ttk.Label(
            scroll_frame1,
            text="No profiles yet. Add samples to get started.",
            font=("Arial", 10),
            foreground="orange",
    ...
    ttk.Label(
        po_frame,
        text="PO and Amount detection patterns per supplier",
        font=("Arial", 10),
        foreground="gray",
    ).pack(pady=5)
    ...
    scrollbar2 = ttk.Scrollbar(po_frame, orient="vertical", command=canvas2.yview)
    ...
    if po_profiles:
        for supplier, profile in po_profiles.items():
            ...
            ttk.Label(
                scroll_frame2,
                text=f"{supplier}: {po_patterns} PO patterns, {amt_patterns} amount patterns, {samples} samples",
                font=("Arial", 10),
            ).pack(anchor=tk.W, pady=5)
    else:
        ttk.Label(
            scroll_frame2,
            text="No PO detection profiles yet.",
            font=("Arial", 10),
            foreground="orange",
        ).pack(anchor=tk.W, pady=10)
    ...
    def add_classification_samples():
        ...
        supplier = simpledialog.askstring(
            "Supplier Code", "Enter supplier code (e.g., AFFI1):", parent=root
        )
        if not supplier:
            return
        ...
        messagebox.showinfo("Success", f"Added {count} samples for supplier {supplier}")
        root.destroy()
    ...
    def add_po_detection():
        ...
        # Ask for PO patterns
        po_pattern_text = simpledialog.askstring(
            "PO Patterns",
        ...
        amt_pattern_text = simpledialog.askstring(
            "Amount Patterns",
    ...
    ttk.Button(
        button_frame,
        text="Add Classification Samples",
        command=add_classification_samples,
    ).pack(side=tk.LEFT, padx=5)
    ttk.Button(
        button_frame, text="Add PO Detection Profile", command=add_po_detection
    ).pack(side=tk.LEFT, padx=5)
    ttk.Button(button_frame, text="Close", command=root.destroy).pack(
        side=tk.LEFT, padx=5
    )
...
def open_batch_learn():
    ...
    files = filedialog.askopenfilenames(
        title="Select PDFs for Batch Learn",
        filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")],
    )
    if not files:
        return
    ...
    info = ttk.Label(
        win,
        text=f"Selected {len(entries)} files for supplier {supplier}",
        font=("Arial", 12),
    )
    info.pack(pady=6)
    ...
    for i, e in enumerate(entries):
        ...
        amt_lbl = ttk.Label(row, text=f"Amt: {e['amount'] if e['amount'] else 'None'}")
        ...
        ttk.Button(row, text="Show OCR", command=make_show(e["text"])).pack(
    ...
    def configure_patterns():
        ...
        ttk.Label(
            config_win, text=f"PO Patterns for {supplier}", font=("Arial", 12, "bold")
        ).pack(pady=10)
        ...
        # PO Patterns
        ttk.Label(
            config_win, text="PO Pattern Labels (one per line):", font=("Arial", 10)
        ...
        ttk.Label(
            config_win,
            text="Examples: 'PO#', '5590', 'PO/Rel'",
            font=("Arial", 9, "italic"),
        ).pack(anchor=tk.W, padx=10)

        # Amount Patterns
        ttk.Label(
            config_win, text="Amount Pattern Labels (one per line):", font=("Arial", 10)
        ...
        ttk.Label(
            config_win,
            text="Examples: 'Total', 'Amount Due', 'Invoice Total'",
            font=("Arial", 9, "italic"),
        ).pack(anchor=tk.W, padx=10)
        ...
        def save_patterns():
            po_patterns = [
                line.strip()
                for line in po_text.get("1.0", tk.END).split("\n")
                if line.strip()
            ]
            amt_patterns = [
                line.strip()
                for line in amt_text.get("1.0", tk.END).split("\n")
                if line.strip()
            ]
        ...
        button_frame.pack(fill=tk.X, padx=10, pady=10)
        ttk.Button(button_frame, text="Save Patterns", command=save_patterns).pack(
            side=tk.LEFT, padx=5
        )
    ...
    ttk.Button(
        btn_frame, text="Save Selected as Samples", command=save_selected, width=30
    ).pack(pady=5, anchor=tk.E)
    ttk.Button(
        btn_frame, text="Configure Patterns", command=configure_patterns, width=30
    ).pack(pady=5, anchor=tk.E)
...
class POConfirmationUI:
    ...
    def create_control_panel(self, parent_frame):
        ...
        frame = ttk.LabelFrame(scrollable_frame, text="PO Number Selection", padding=8)
        frame.pack(fill=tk.BOTH, expand=True, padx=3, pady=3)

        # Extracted POs display
        if self.extracted_pos:
            ttk.Label(
                frame, text="Extracted PO options:", font=("Arial", 10, "bold")
            ...
            for po in self.extracted_pos:
                ...
                ttk.Label(btn_frame, text="", font=("Arial", 9)).pack(
                    side=tk.LEFT, padx=5
                )
        else:
            ttk.Label(
                frame,
                text="No PO numbers automatically detected",
                foreground="orange",
                font=("Arial", 10),
            ).pack(anchor=tk.W, pady=(0, 5))
        ...
        ttk.Label(
            frame,
            text="Or enter manually (5 digits: 40085 or 7 digits: 0040085, optional -dd):",
            font=("Arial", 10),
        ).pack(anchor=tk.W, pady=(0, 5))
        ...
        self.manual_entry = ttk.Entry(entry_frame, width=30, font=("Arial", 10))
        self.manual_entry.pack(side=tk.LEFT, padx=5)

        # Amount entry
        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)

        ttk.Label(frame, text="Enter Amount (optional):", font=("Arial", 10)).pack(
        ...
        ttk.Button(amount_frame, text="Save", command=self.confirm_manual).pack(
            side=tk.LEFT, padx=2
        )
        ...
        ttk.Label(frame, text="Check No (optional):", font=("Arial", 10)).pack(
            anchor=tk.W, pady=(0, 5)
        )
        ...
        self.check_no_entry = ttk.Entry(check_no_frame, width=20, font=("Arial", 10))
        self.check_no_entry.pack(side=tk.LEFT, padx=5)

        # ReceiverID entry
        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)

        ttk.Label(frame, text="ReceiverID (optional):", font=("Arial", 10)).pack(
            anchor=tk.W, pady=(0, 5)
        )
        ...
        ttk.Button(button_frame, text="Skip File (S)", command=self.skip_file).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            button_frame, text="Mark as No PO (N)", command=self.confirm_no_po
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            button_frame,
            text="Skip Remaining (Shift+S)",
            command=self.skip_remaining_batch,
        ).pack(side=tk.LEFT, padx=5)

        # Keyboard shortcuts help
        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)
        help_text = "Keyboard: S = Skip, N = No PO, Shift+S = Skip Remaining, Enter = Confirm Manual"
        ttk.Label(frame, text=help_text, font=("Arial", 8), foreground="gray").pack(
            anchor=tk.W, pady=5
        )
    ...
    def confirm_manual(self):
        ...
        if not re.match(r"^([0-9]{5}|0{2}[0-9]{5})(-\d{2})?$", po):
            messagebox.showerror(
                "Invalid Format",
                "PO must be 5 digits (e.g., 40085) or 7 digits with leading 00 (e.g., 0040085), optionally with -dd suffix",
            )
            return
        # Strip leading zeros
        if "-" in po:
...
def create_manual_entry_ui(
    ...
    ttk.Label(info_frame, text=f"File: {filename}", font=("Arial", 10, "bold")).pack(
        anchor=tk.W
    )

    # Extracted candidates
    cand_text = ", ".join(po_candidates) if po_candidates else "None found"
    ttk.Label(info_frame, text=f"PO Candidates: {cand_text}", font=("Arial", 9)).pack(
        anchor=tk.W
    )
    ...
    if po_candidates:
        ...
        for po in po_candidates:
            ttk.Button(
                candidates_frame, text=po, width=10, command=make_select_po(po)
    ...
    receiver_entry = ttk.Entry(entry_frame, textvariable=receiver_var, width=20)
    ...
    ttk.Button(
        button_frame, text="Save & Continue", command=save_and_close, width=18
    ).pack(pady=5)
...
def process_batch_improved(batch_size: int = 10) -> tuple:
    ...
    print(f"Total processed: {len(unprocessed)} / {len(pdf_files)} files remaining")

    return processed_count, complete_count, incomplete_count, error_count


def process_batch(
    batch_size: int = 10, start_from: int = 0, with_confirmation: bool = True
) -> tuple:
    """
    ...
    for i, pdf_path in enumerate(batch, 1):
        ...
        try:
            ...
            if with_confirmation:
                ...
                confirmed_po, amount, check_no, receiver_id, skipped, skip_remaining = (
                    ui.show()
                )
...
def test_single_file():
    ...
    ttk.Label(root, text="Test PO/Amount Detection", font=("Arial", 14, "bold")).pack(
        pady=10
    )
    ...
    def run_test():
        ...
        result_text.insert(tk.END, f"Supplier: {supplier if supplier else 'Unknown'}\n")
        ...
        try:
            ...
            po_results = extract_po_with_supplier_profile(extracted_text, supplier)
            result_text.insert(tk.END, f"PO Detection Results:\n")
            result_text.insert(
                tk.END,
                f"  Extracted POs: {po_results if po_results else 'None found'}\n\n",
            )
    ...
    def test_location_based():
        ...
        result_text.insert(tk.END, f"Supplier: {supplier if supplier else 'Unknown'}\n")
        result_text.insert(tk.END, "=" * 60 + "\n")
        result_text.insert(tk.END, "LOCATION-BASED EXTRACTION (Bounding Boxes)\n")
        result_text.insert(tk.END, "=" * 60 + "\n\n")
        ...
        try:
            ...
            if not has_amount_location(supplier):
                ...
                result_text.insert(
                    tk.END,
                    f"Amount (from location): {amount if amount else 'None found'}\n",
                )
    ...
    def show_ocr_text():
        ...
        if extracted_text is None:
            messagebox.showinfo("Info", "Please run test first to extract text")
            return
        ...
        ttk.Label(
            ocr_window,
            text="Full Extracted Text (for pattern development)",
            font=("Arial", 12, "bold"),
        ).pack(pady=10)
        ...
        ttk.Button(button_frame, text="Copy All Text", command=copy_all).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Close", command=ocr_window.destroy).pack(
            side=tk.LEFT, padx=5
        )
    ...
    result_text = tk.Text(results_frame, height=20, width=100, state=tk.DISABLED)
    ...
    test_location_button = ttk.Button(
        button_frame,
        text="Test Location-Based",
        command=test_location_based,
        state=tk.DISABLED,
    )
...
def test_location_extraction():
    ...
    # Configure tesseract OCR first
    configure_tesseract()

    root = tk.Tk()
    root.title("Test Location Mapping")
    root.geometry("900x600")

    ttk.Label(
        root, text="Test Location-Based Extraction", font=("Arial", 14, "bold")
    ).pack(pady=10)
    ...
    def run_test():
        ...
        from location_extraction.config import load_config

        # Get supplier code
        supplier_code = simpledialog.askstring(
            "Supplier Code", "Enter supplier code:", parent=root
        )
        if not supplier_code:
            return
        ...
        result_text.insert(tk.END, f"File: {os.path.basename(selected_file)}\n")
        result_text.insert(tk.END, f"Supplier: {supplier_code}\n")
        result_text.insert(tk.END, "=" * 60 + "\n\n")

        try:
            from location_extraction.extractor import (
                extract_po_from_location,
                extract_po_from_location_debug,
                extract_amount_from_location,
                extract_amount_from_location_debug,
                extract_invoice_from_location,
            )
            from location_extraction.config import get_supplier_location

            location = get_supplier_location(supplier_code)

            # Test PO extraction
            if "po" in location:
                result_text.insert(tk.END, f"PO Location: {location['po']}\n")
                po, raw_po_text = extract_po_from_location_debug(
                    selected_file, supplier_code
                )
                result_text.insert(tk.END, f"Raw OCR Text: {repr(raw_po_text[:100])}\n")
                result_text.insert(
                    tk.END, f"Extracted PO: {po if po else 'NOT FOUND'}\n\n"
                )
            else:
                result_text.insert(tk.END, "No PO location configured\n\n")

            # Test Amount extraction
            if "amount" in location:
                result_text.insert(tk.END, f"Amount Location: {location['amount']}\n")
                amount, raw_amount_text = extract_amount_from_location_debug(
                    selected_file, supplier_code
                )
                result_text.insert(
                    tk.END, f"Raw OCR Text: {repr(raw_amount_text[:100])}\n"
                )
                result_text.insert(
                    tk.END, f"Extracted Amount: {amount if amount else 'NOT FOUND'}\n\n"
                )
            else:
                result_text.insert(tk.END, "No Amount location configured\n\n")

            # Test Invoice extraction
            if "invoice" in location:
                result_text.insert(tk.END, f"Invoice Location: {location['invoice']}\n")
                invoice = extract_invoice_from_location(selected_file, supplier_code)
                result_text.insert(
                    tk.END,
                    f"Extracted Invoice: {invoice if invoice else 'NOT FOUND'}\n\n",
                )
            else:
                result_text.insert(tk.END, "No Invoice location configured\n\n")
    ...
    # Results frame
    results_frame = ttk.LabelFrame(root, text="Location Extraction Results", padding=10)
    results_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
...
def main(use_ui: bool = True):
    ...
    ttk.Label(root, text="Select a workflow:", font=("Arial", 12)).pack(pady=10)
    ...
    def launch_classifier():
        ...
        # Buttons
        tk.Button(app_root, text="Add Sample Invoices", command=add_samples).pack()
        tk.Button(
            app_root, text="Classify New Invoices", command=classify_invoices
        ).pack()
        ...
        def add_samples():
            ...
            if supplier and files:
                for f in files:
                    ...
                    profiles.setdefault(supplier, []).append(text)
                save_profiles(profiles)
                log.insert(tk.END, f"Added {len(files)} samples for {supplier}\n")
        ...
        def classify_invoices():
            ...
            vectorizer, sample_vectors, sample_labels = train_supplier_profiles(
                profiles
            )
    ...
    ttk.Button(
        button_frame,
        text="2. Build PO/Amount Profiles\n(Train Detection Patterns)",
        command=launch_profile_manager,
        width=40,
    ).pack(pady=10)

    ttk.Button(
        button_frame,
        text="3. Test Single File\n(Validate Patterns)",
        command=launch_test,
        width=40,
    ).pack(pady=10)
    ...
    ttk.Button(
        button_frame,
        text="4. Batch Learn & Validate\n(Multi-select samples)",
        command=launch_batch_learn,
        width=40,
    ).pack(pady=10)

    ttk.Button(
        button_frame,
        text="5. Extract PO/Amount (Legacy)\n(Apply Detection Profiles)",
        command=launch_po_extraction,
        width=40,
    ).pack(pady=10)

    ttk.Button(
        button_frame,
        text="6. Extract with Supplier Matching (NEW)\n(Improved workflow)",
        command=launch_improved_extraction,
        width=40,
    ).pack(pady=10)
    ...
    ttk.Button(
        button_frame,
        text="7. Configure Location Mapping (NEW)\n(Calibrate supplier invoice layout)",
        command=launch_location_extraction,
        width=40,
    ).pack(pady=10)

    def launch_location_test():
        """Test location-based extraction on single file."""
        root.destroy()
    ...
    ttk.Button(
        button_frame,
        text="8. Test Location Mapping (NEW)\n(Validate calibrated locations)",
        command=launch_location_test,
        width=40,
    ).pack(pady=10)
    ...
    ttk.Button(button_frame, text="Exit", command=exit_app, width=40).pack(pady=10)
    ...
    while True:
        ...
        if status["remaining"] > 0:
            ...
            continue_processing = messagebox.askyesno(
                "Continue Processing", "Batch complete! Process another batch?"
            )
            root.destroy()
