...
from location_extraction.config import get_supplier_location, has_po_location, has_amount_location
...
def extract_text_from_region(pdf_path: str, x0: float, y0: float, x1: float, y1: float, page: int = 0) -> str:
    """
    ...
    try:
        ...
        cropped_page = pdf_page.get_pixmap(
            clip=crop_box, matrix=fitz.Matrix(2, 2))  # 2x zoom for better OCR
...
def extract_po_and_amount_from_location(pdf_path: str, supplier_code: str) -> Tuple[Optional[str], Optional[str]]:
    """
